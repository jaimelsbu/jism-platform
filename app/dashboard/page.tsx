...
// ─────────────────────────────────────────────────────────────────────────────
// VEKTORQ Private Dashboard
// Shows analytics for all client sites.
// Protected by a simple password (no auth library needed at this stage).
// Visit: vektorq.com/dashboard
// ─────────────────────────────────────────────────────────────────────────────
...
type Site = {
  ...
  client_name: string; plan: string; sessions_30d: number;
  trial_ends?: string;
};
...
export default function DashboardPage() {
  ...
  // Load site data when active site or days changes
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <a href="/" style={{ fontSize: 12, color: '#424754', textDecoration: 'none' }}>← Site</a>
          </div>
        </div>
      </div>
        ...
        {sites.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ color: '#9ca3b0', fontSize: 14, maxWidth: 400, margin: '0 auto 24px' }}>
              Add your first client site below, then give them the tracking snippet to install.
            </p>
            <a href="/dashboard/add-site" style={{
              display: 'inline-block', padding: '12px 24px',
              background: 'linear-gradient(135deg,#4d8eff,#9b59ff)',
              color: '#fff', fontWeight: 700, fontSize: 14,
              borderRadius: 10, textDecoration: 'none',
            }}>
              + Add First Client Site
            </a>
          </div>
        )}
              ...
              <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                <div>
                  ...
                  <p style={{ fontSize: 13, color: '#9ca3b0', margin: '3px 0 0' }}>
                    {sites.find(s => s.id === activeSite)?.domain} ·{' '}
                    <span style={{ color: '#34d399' }}>
                      {sites.find(s => s.id === activeSite)?.plan} plan
                    </span>
                  </p>
                </div>
              ...
              {(['overview', 'pages', 'alerts', 'funnel'] as const).map(t => (
                <button
                  ...
                  style={{
                    ...
                    color: tab === t ? '#fff' : '#9ca3b0',
                    fontSize: 13, fontWeight: tab === t ? 600 : 400,
                    cursor: 'pointer', textTransform: 'capitalize',
                    marginBottom: -1,
                  }}
                >
                  {t === 'alerts' && (data?.rage?.length || data?.dead?.length)
                    ? `⚠ ${t}` : t}
                </button>
              ))}
            </div>
